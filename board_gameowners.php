...
<body id="main_body" >
	...
	<div id="form_container">
		...
		</div><p class="guidelines" id="email"><small>Enter a Location ID between 1 & 100</small></p>
		</li>		
		<li id="li_2" >
		<label class="description" for="memberID">Member ID </label>
		<div>
			<input id="memberID" name="memberID" class="element text medium" type="text" maxlength="255" min="1" max="999" pattern="\d{1,3}" title="Requires a 3 location ID between 1 & 999" value="<?php if (isset($locID)) { echo $locID; } ?>"/>
			<label>Member that the game is located with</label>
		</div><p class="guidelines" id="email"><small>Enter a MemberID</small></p>
		</li>	
		<li id="li_1" >
		<label class="description" for="gamename">GameName </label>
		<span>
			<input id="gamename" name= "gamename" class="element text" maxlength="255" size="50" title="Name of the game" value="<?php if (isset($gamename)) { echo $gamename; } ?>"/>
		...
		<li class="buttons">
			...
			<button type="button" onclick="location.href='/board_gameowners.php';">Reset Form</button>
